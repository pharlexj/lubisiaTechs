...
import { useListAboutSections, useListTeamMembers } from "@workspace/api-client-react";
import type { TeamMember, AboutSection } from "@workspace/api-client-react";
import { ChevronLeft, ChevronRight, Linkedin, Target, Eye, BookOpen, Shield, Users } from "lucide-react";
...
function TeamCarousel({ members }: { members: TeamMember[] }) {
  ...
  const resetTimer = () => {
    ...
    timerRef.current = setInterval(() => setIndex((i) => (i + 1) % active.length), 5000);
  };
  ...
  const prev = () => { setIndex((i) => (i - 1 + active.length) % active.length); resetTimer(); };
  const next = () => { setIndex((i) => (i + 1) % active.length); resetTimer(); };

  if (active.length === 0) return null;

  const visible = Array.from({ length: visibleCount }, (_, offset) => active[(index + offset) % active.length]);
    ...
    <div className="relative">
      ...
      {active.length > visibleCount && (
        <div className="flex items-center justify-center gap-4 mt-8">
          <Button variant="outline" size="icon" onClick={prev} className="rounded-full border-primary text-primary hover:bg-primary hover:text-white">
          ...
          <Button variant="outline" size="icon" onClick={next} className="rounded-full border-primary text-primary hover:bg-primary hover:text-white">
...
export function About() {
  ...
  const sectionMap = Object.fromEntries(sections.map((s: AboutSection) => [s.key, s]));
      ...
      <section className="relative bg-gradient-to-br from-primary via-primary/90 to-accent py-24 overflow-hidden">
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          ...
          <h1 className="text-4xl md:text-6xl font-extrabold text-white mb-6 leading-tight">
            About LubisiaTeks<br />
            <span className="text-secondary">Solutions</span>
          </h1>
          <p className="text-lg md:text-xl text-white/80 max-w-2xl mx-auto">
            Empowering businesses and individuals across Kenya through innovative IT services and quality computer accessories.
          </p>
        </div>
      </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {["vision", "mission", "mandate"].map((key, i) => {
                ...
                return (
                  ...
                  >
                    <div className={`h-14 w-14 rounded-xl flex items-center justify-center mb-5 ${SECTION_COLORS[key]}`}>
                      {SECTION_ICONS[key]}
                    </div>
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-3xl mx-auto">
              <div data-reveal="down" className="text-center mb-10">
                ...
                <h2 className="text-3xl font-extrabold text-primary">{sectionMap.core_values.title}</h2>
                <p className="text-muted-foreground mt-2">The principles that guide everything we do</p>
              </div>
              <div data-reveal="up" data-delay="150" className="hover-up bg-white rounded-2xl shadow-md p-8 border border-border">
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center text-secondary-foreground">
            <div data-reveal="up" data-delay="0">
              ...
              <div className="text-sm mt-1 text-white/80 font-medium">Years Experience</div>
            </div>
            <div data-reveal="up" data-delay="150">
              <div className="text-4xl font-extrabold">
                <AnimatedNumber to={500} suffix="+" />
              </div>
              <div className="text-sm mt-1 text-white/80 font-medium">Happy Clients</div>
            </div>
            <div data-reveal="up" data-delay="300">
              <div className="text-4xl font-extrabold">
                <AnimatedNumber to={8} />
              </div>
              <div className="text-sm mt-1 text-white/80 font-medium">IT Services</div>
            </div>
            <div data-reveal="up" data-delay="450">
              <div className="text-4xl font-extrabold">Kitale</div>
              <div className="text-sm mt-1 text-white/80 font-medium">Based In Kenya</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <div data-reveal="up" className="text-center mb-12">
            <Badge className="mb-3 bg-accent/10 text-accent border-accent/20">Our People</Badge>
            <h2 className="text-3xl md:text-4xl font-extrabold text-primary">Meet the Team</h2>
            <p className="text-muted-foreground mt-3 max-w-xl mx-auto">
              The passionate professionals behind LubisiaTeks Solutions
            </p>
          </div>
          {membersLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
